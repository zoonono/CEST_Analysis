...
BS_RAW_sum_roi = zeros(input.number_of_scans,cest_scan(1).number_of_ppm_values); 
...
BS_interpolated_mean_Rsqr_sum_roi = zeros(input.number_of_scans,length(interpolated_sum_roi(1).ppm));
for ii = 1 : input.number_of_scans
...
%sum_roi
%interpolated mean-of-raw-baselines subtraction (1rst approach)
BS_interpolated_mean_sum_roi = zeros(input.number_of_scans,length(interpolated_sum_roi(1).ppm));
for ii = 1 : input.number_of_scans
 for jj = 1 : length(interpolated_sum_roi(1).ppm)
...
BS_RAW_rois = zeros(masks.number_of_rois, input.number_of_scans, cest_scan(1).number_of_ppm_values);
BS_interpolated_mean_Rsqr_rois = zeros(masks.number_of_rois,input.number_of_scans,length(interpolated_sum_roi(1).ppm));
...
% for pixels %
%%%%%%%%%%%%%%
BS_RAW_pixels = zeros(input.number_of_scans, ...
                      cest_scan(1).number_of_ppm_values,...
                      size(interpolated_pixels,2));
BS_interpolated_mean_pixels = zeros(input.number_of_scans,...
